...
def test_engine_rejects_order_over_limit():
    ...
    with pytest.raises(ValueError, match="max order notional"):
        engine.execute(Order(make_asset(), OrderSide.BUY, Decimal("1"), OrderType.MARKET))


def test_low_confidence_signal_is_not_executed():
    broker = PaperBroker({"BTC-USD": Decimal("100")})
    engine = TradingEngine(PlatformConfig(supported_assets=(make_asset(),)), broker, broker)
    with pytest.raises(ValueError, match="confidence"):
        engine.execute_signal(
            Signal("BTC-USD", OrderSide.BUY, Decimal("0.49")), Decimal("0.1"), make_asset()
        )
...
def test_kill_switch_halts_orders():
    ...
    with pytest.raises(ValueError, match="kill switch"):
        engine.execute(Order(make_asset(), OrderSide.BUY, Decimal("0.1"), OrderType.MARKET))
